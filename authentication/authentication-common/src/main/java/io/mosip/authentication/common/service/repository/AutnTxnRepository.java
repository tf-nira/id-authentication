...
public interface AutnTxnRepository extends BaseRepository<AutnTxn, Integer> {
	...
	@Query(value = "Select new AutnTxn( requestTrnId, requestDTtimes, authTypeCode, statusCode, statusComment, refId, refIdType, entityName, requestSignature, responseSignature,amount ) from AutnTxn where token=:token ORDER BY crDTimes DESC")
